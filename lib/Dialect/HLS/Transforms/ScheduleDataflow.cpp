...
struct ScheduleDataflow
  ...
  void runOnOperation() override {
    ...
    // Localize stream view ops. The rationale is that task op should never
    // yield a viewed itensor - it must directly yield the itensor generated by
    // a destination-style op, e.g. a sub-task, a itensor write op, etc.
    // Therefore, a view op should always be localized before doing the
    // scheduling to maintain the semantics of task op.
    for (auto view : llvm::make_early_inc_range(
             func.getOps<hls::ITensorViewLikeOpInterface>())) {
      for (auto &use : llvm::make_early_inc_range(view->getUses())) {
        builder.setInsertionPoint(
            getParentInBlock(use.getOwner(), view->getBlock()));
        auto localView =
            cast<hls::ITensorViewLikeOpInterface>(builder.clone(*view));
        use.set(localView.getResult());
      }
      view.erase();
    }
