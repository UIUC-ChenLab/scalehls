//===- FoldMemrefSubview.cpp - Fold memref.subview ops
//-----------------------===//
//
...
resolveSourceIndices(Location loc, PatternRewriter &rewriter,
  ...
  llvm::outs() << subViewOp << "\n";
  llvm::outs() << indices.size() << "\n";
  llvm::SmallBitVector unusedDims = subViewOp.getDroppedDims();
  uintptr_t data;
  for (auto i : unusedDims.getData(data))
    llvm::outs() << i << ",";
  llvm::outs() << "\n";
...
static void populateFoldMemrefSubviewPatterns(RewritePatternSet &patterns) {
...
struct FoldMemrefSubviewPass final
    : public FoldMemrefSubviewBase<FoldMemrefSubviewPass> {
  void runOnOperation() override;
};

} // namespace

void FoldMemrefSubviewPass::runOnOperation() {
  RewritePatternSet patterns(&getContext());
  populateFoldMemrefSubviewPatterns(patterns);
  (void)applyPatternsAndFoldGreedily(getOperation(), std::move(patterns));
}

std::unique_ptr<Pass> scalehls::createFoldMemrefSubviewPass() {
  return std::make_unique<FoldMemrefSubviewPass>();
}
