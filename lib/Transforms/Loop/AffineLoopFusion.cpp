...
struct LoopFusion : public AffineLoopFusionBase<LoopFusion> {
  ...
  LoopFusion(double computeToleranceThreshold, unsigned fastMemorySpace,
             ...
             enum AffineFusionMode affineFusionMode) {
...
std::unique_ptr<Pass> scalehls::createAffineLoopFusionPass(
    double computeToleranceThreshold, unsigned fastMemorySpace,
...
public:
  ...
  Operation *getFusedLoopNestInsertionPoint(unsigned srcId, unsigned dstId) {
    ...
    if (firstSrcDepPos.value()) {
      if (lastDstDepPos.value()) {
...
static Value createPrivateMemRef(AffineForOp forOp, Operation *srcStoreOpInst,
  ...
  assert(numElements.value() && "non-constant number of elts in local buffer");
...
static bool isFusionProfitable(Operation *srcOpInst, Operation *srcStoreOpInst,
  ...
  if (!maybeSrcWriteRegionSizeBytes.value())
    return false;
  ...
  for (unsigned i = maxLegalFusionDepth; i >= 1; --i) {
    ...
    if (!maybeSliceWriteRegionSizeBytes.value() ||
  ...
  *dstLoopDepth = bestDstLoopDepth.value();
  ...
  LLVM_DEBUG({
    ...
    msg << (storageReduction.value() ? std::to_string(storageReduction.value())
                                     : "<unknown>");
...
public:
  ...
  bool findSiblingNodeToFuse(Node *dstNode,
    ...
    for (unsigned i = 0, e = fn.getBody().front().getNumArguments(); i != e;
         ++i) {
      for (auto *user : fn.getBody().front().getArgument(i).getUsers()) {
...
void LoopFusion::runOnOperation() {
  getOperation().walk([&](TaskOp task) {
    ...
    if (affineFusionMode == AffineFusionMode::ProducerConsumer)
      fusion.runProducerConsumerFusionOnly();
    else if (affineFusionMode == AffineFusionMode::Sibling)
