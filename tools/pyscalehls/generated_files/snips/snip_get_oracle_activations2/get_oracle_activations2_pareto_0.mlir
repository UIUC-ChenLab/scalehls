func @get_oracle_activations2(%arg0: memref<192xf64, affine_map<(d0) -> (d0 mod 24, d0 floordiv 24)>, 1>, %arg1: memref<3xf64, affine_map<(d0) -> (d0 mod 3, d0 floordiv 3)>, 1>, %arg2: memref<64xf64, affine_map<(d0) -> (d0 mod 8, d0 floordiv 8)>, 1>, %arg3: memref<64xf64, affine_map<(d0) -> (d0 mod 8, d0 floordiv 8)>, 1>) attributes {func_directive = #hlscpp.fd<pipeline=false, targetInterval=1, dataflow=false, topFunc=true>, llvm.linkage = #llvm.linkage<external>, resource = #hlscpp.r<lut=0, dsp=72, bram=0>, timing = #hlscpp.t<0 -> 44, 44, 44>} {
  ...
  } {loop_directive = #hlscpp.ld<pipeline=true, targetII=2, dataflow=false, flatten=false, parallel=true>, loop_info = #hlscpp.l<flattenTripCount=8, iterLatency=26, minII=2>, timing = #hlscpp.t<0 -> 42, 42, 42>}
  return {timing = #hlscpp.t<42 -> 42, 0, 0>}
}
