
//===------------------------------------------------------------*- C++ -*-===//
//
// Automatically generated file for High-level Synthesis (HLS).
//
//===----------------------------------------------------------------------===//



/// This is top function.
void kernel_2mm(
  float v0,
  float v1,
  float v2[64][64],
  float v3[64][64],
  float v4[64][64],
  float v5[64][64],
  float v6[64][64]
) {	// L2






  float v7 = 0;	// L4
  loop0: for (int v8 = 0; v8 < 64; v8 += 1) {	// L8
    loop1: for (int v9 = 0; v9 < 64; v9 += 1) {	// L5
      loop2: for (int v10 = 0; v10 < 64; v10 += 1) {	// L6
        if (v8 == 0) {	// L6
          v2[v9][v10] = v7;	// L7
        }
        float v11 = v3[v9][v8];	// L9
        float v12 = v0 * v11;	// L10
        float v13 = v4[v8][v10];	// L11
        float v14 = v12 * v13;	// L12
        float v15 = v2[v9][v10];	// L13
        float v16 = v15 + v14;	// L14
        v2[v9][v10] = v16;	// L15
      }
    }
  }
  loop3: for (int v17 = 0; v17 < 64; v17 += 1) {	// L24
    loop4: for (int v18 = 0; v18 < 64; v18 += 1) {	// L19
      loop5: for (int v19 = 0; v19 < 64; v19 += 1) {	// L20
        float v20 = v6[v18][v19];	// L21
        float v21 = v20 * v1;	// L22
        if (v17 == 0) {	// L20
          v6[v18][v19] = v21;	// L23
        }
        float v22 = v2[v18][v17];	// L25
        float v23 = v5[v17][v19];	// L26
        float v24 = v22 * v23;	// L27
        float v25 = v6[v18][v19];	// L28
        float v26 = v25 + v24;	// L29
        v6[v18][v19] = v26;	// L30
      }
    }
  }
}

