...
private:
  ...
  std::string typePrint(QualType Type) {
    ...
    if (isIndex(Type))
      ...
      return "i32"; // i" + std::to_string(Context->getTypeSize(Type));
  ...
  int binaryOperatorGen(clang::Stmt *Stmt, int resultVar,
    ...
    if (BinaryOp->getOpcodeStr() == "=") {
      ...
      if (resultVar != -1) {
        ...
        assert(getVarType(resultVar) == typeIn &&
               "Cast for binary op not supported");
  ...
  int castTo(int var, int mode, QualType ty, std::string *exprBuff) {
    ...
    case 10: // int2index
      ...
      *exprBuff += getVarName(res) + " = index_cast " + getVarName(var) +
      ...
      break;
    case 2: // index2float
      error("Undefined type conversion in castTo.");
      break;
    case 12: // int2float
      error("Undefined type conversion in castTo.");
      break;
  ...
  int unaryExprGen(clang::Stmt *Stmt, int resultVar, std::string *exprBuff) {
    ...
    case clang::UnaryOperator::Opcode::UO_PostInc:
      assert(typeIn->isIntegerType() &&
             "unary op \"++\" not supported for floating types");
      *exprBuff += incrName(getVarName(var)) + " = addi " + getVarName(var) +
                   ", " + getVarName(constIntGen(1, typeIn, exprBuff)) + " : " +
                   typePrint(typeIn) + "\n";
    ...
    case clang::UnaryOperator::Opcode::UO_PostDec:
      assert(typeIn->isIntegerType() &&
             "unary op \"--\" not supported for floating types");
      *exprBuff += incrName(getVarName(var)) + " = subi " + getVarName(var) +
                   ", " + getVarName(constIntGen(1, typeIn, exprBuff)) + " : " +
                   typePrint(typeIn) + "\n";
      assert(std::find(unaryUpdateList.begin(), unaryUpdateList.end(), var) ==
                 unaryUpdateList.end() &&
  ...
  int findCastMode(QualType dst, QualType src) {
    ...
    if (srcType == "i32" && dstType == "index") // int2index
      return 10;
    else if (srcType == "index" && dstType == "i32") // index2int
      return 1;
    else
      error("Unsupported conversion : " + srcType + " -> " + dstType);
    return -1;
  }
    ...
    case Stmt::StmtClass::DeclRefExprClass:
      if (resultVar == -1)
        ...
        *exprBuff += incrVarName(resultVar) + " = add" + isInt + " " +
                     getVarName(constIntGen(0, ty, exprBuff)) + ", " +
                     getVarName(var) + " : " + typePrint(ty) + "\n";
        result = resultVar;
      }
      break;
  ...
  void stmtGen(Stmt *Stmt, std::string *stmtBuff) {
    ...
    case Stmt::StmtClass::IfStmtClass:
      ...
      break;
    case Stmt::StmtClass::ReturnStmtClass:
      assert(std::distance(std::cbegin(children), std::cend(children)) == 1);
  ...
  void funcGen(FunctionDecl *func) {
    std::string funcBody;
    mlirOut << "func @" << func->getNameInfo().getName().getAsString() << "(";
...
void exit(void) {
  std::cout << "Usage: scalehls-clang [options] <source>" << std::endl;
