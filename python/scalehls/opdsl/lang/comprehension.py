    ...
    def __init__(self, value: Any):
        with _ir.Context():
            if isinstance(value, float):
                ...
                self.value = str(
                    _ir.IntegerAttr.get(
                        _ir.IntegerType.get_signless(64), int(value))
                )
            else:
                raise ValueError(
                    f"const requires int or float but got {type(value)}")
    ...
    def __init__(
        ...
        *reduce_dims: DimDef,
        ...
        if bool(binary_fn) + bool(binary_attr) != 1:
            raise ValueError(
                "One of 'binary_fn', 'binary_attr' must be specified")
...
class ReduceFnType:
    ...
    def __init__(self, binary_fn: BinaryFnType):
        if not isinstance(binary_fn, BinaryFnType):
            raise ValueError(
                f"Reduce expected a BinaryFnType but got {binary_fn}")
        self.binary_fn = binary_fn
...
class OperandDef:
    ...
    def attach(self, index: int, name: str, owner: "LinalgOpDef"):
        if self.owner:
            raise ValueError(
                f"OperandDef already registered with an op: {self}")
    ...
    def __init__(self, *bindings: Tuple[TensorUse, TensorExpression]):
        ...
        for assign, value in bindings:
            if isinstance(value, TensorReduceFn):
                if value.lhs:
                    raise ValueError(
                        f"Reduction expression already assigns: {value}")
                value.lhs = assign
...
class LinalgOpDef:
    ...
    def add_operand(self, name: str, operand: OperandDef):
        ...
        if operand.is_input() and any(
            ...
            raise ValueError(
                f"Input {name} registered after an output or attribute")
