...
def prepare_common_structured_op(
    ...
    **attrs: Union[Sequence[int], TypeFnType],
    ...
    out_arg_defs = [d for d in all_arg_defs if d.kind ==
                    OperandKind.OUTPUT_TENSOR]
    index_attr_arg_defs = [
    ...
    for curr in op_config.indexing_maps:
        for expression, replacement in zip(expressions, replacements):
            curr = curr.replace(expression, replacement,
                                curr.n_dims, curr.n_symbols)
    ...
    indexing_maps = AffineMap.compress_unused_symbols(
        indexing_maps, Context.current)
    ...
    for arg_def, arg_element_type in zip(
        ...
        _add_type_mapping(arg_def, arg_element_type,
                          type_mapping, block_arg_types)
    ...
    **attrs: Sequence[int],
    ...
    with InsertionPoint(block):
        body_builder = _BodyBuilder(
            type_mapping, block_arg_mapping, fn_attr_mapping)
    ...
    def expression(self, expr: ScalarExpression) -> Value:
        if expr.scalar_arg:
            ...
            if expr.scalar_fn.kind == FunctionKind.TYPE:
                operand_values = [
                    expr.scalar_fn.type_var.name] + operand_values
...
def _add_type_mapping(
    ...
    if operand_config.shape_map:
        try:
            ...
            raise ValueError(
                f"Expected ShapedType but got {operand_type}") from e
