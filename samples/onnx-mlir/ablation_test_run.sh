...
do
  ...
  case $n in
    ...
    2)  scalehls-opt $file $legalize "$graph_O6" $split $hlscpp $pipeline $direct | scalehls-translate -emit-hlscpp -o $output ;;
    # Loop (level 6) + directive opt.
    3)  scalehls-opt $file $legalize $split $hlscpp $perfect "$loop_O6" $direct | scalehls-translate -emit-hlscpp -o $output ;;
    ...
    10) scalehls-opt $file $legalize "$graph_O1" $split $hlscpp $perfect "$loop_O6" $direct | scalehls-translate -emit-hlscpp -o $output ;;
    11) scalehls-opt $file $legalize "$graph_O2" $split $hlscpp $perfect "$loop_O6" $direct | scalehls-translate -emit-hlscpp -o $output ;;
    12) scalehls-opt $file $legalize "$graph_O3" $split $hlscpp $perfect "$loop_O6" $direct | scalehls-translate -emit-hlscpp -o $output ;;
    13) scalehls-opt $file $legalize "$graph_O4" $split $hlscpp $perfect "$loop_O6" $direct | scalehls-translate -emit-hlscpp -o $output ;;
    14) scalehls-opt $file $legalize "$graph_O5" $split $hlscpp $perfect "$loop_O6" $direct | scalehls-translate -emit-hlscpp -o $output ;;
  esac
