...
# Please run: source ablation_test_run.sh -m resnet18 -n 8 -c 0
...
# Ablation test.
echo -e "Name\tBRAM\tDSP\tLUT\tCycles\tInterval" > ${model_name}/test_result.log
n=0
while [ $n -lt $ablation_number ]
do
  file=${model_name}/${model_name}.mlir
  output=${model_name}/cpp_src/ablation_$n.cpp
  ...
  then
    ...
    vivado_hls ../../hls_script.tcl "main_graph" "../cpp_src/ablation_$n.cpp" "ablation_$n"
    cd ../..
  fi

  csynth_xml=${model_name}/hls_proj/ablation_$n/main_graph/syn/report/csynth.xml
  ...
  echo -e "Case_$n\t$bram\t$dsp\t$lut\t$cycles\t$interval" >> ${model_name}/test_result.log
