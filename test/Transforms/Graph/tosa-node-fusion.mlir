...
module {
  ...
  func @forward(%arg0: tensor<1x3x32x32xi8>) -> tensor<1x10xi8> {
    ...
    // CHECK: %8 = hls.dataflow.node() -> tensor<1x32x32x64xi8> {
    ...
    // CHECK:   "hls.dataflow.output"(%17) : (tensor<1x32x32x64xi8>) -> ()
    // CHECK: }
    ...
    // CHECK: %9 = hls.dataflow.node() -> tensor<1x32x32x64xi8> {
    // CHECK:   %15 = "tosa.conv2d"(%8, %3, %2) {dilation = [1, 1], pad = [1, 1, 1, 1], quantization_info = {input_zp = 0 : i32, weight_zp = 0 : i32}, stride = [1, 1]} : (tensor<1x32x32x64xi8>, tensor<64x3x3x64xi8>, tensor<64xi8>) -> tensor<1x32x32x64xi8>
    ...
    // CHECK:   %15 = "tosa.conv2d"(%9, %4, %2) {dilation = [1, 1], pad = [1, 1, 1, 1], quantization_info = {input_zp = 0 : i32, weight_zp = 0 : i32}, stride = [1, 1]} : (tensor<1x32x32x64xi8>, tensor<64x3x3x64xi8>, tensor<64xi8>) -> tensor<1x32x32x64xi8>
    ...
    // CHECK:   %15 = "tosa.add"(%10, %8) : (tensor<1x32x32x64xi8>, tensor<1x32x32x64xi8>) -> tensor<1x32x32x64xi8>
    // CHECK:   %16 = "tosa.clamp"(%15) {max_fp = 3.40282347E+38 : f32, max_int = 2147483647 : i64, min_fp = 0.000000e+00 : f32, min_int = 0 : i64} : (tensor<1x32x32x64xi8>) -> tensor<1x32x32x64xi8>
    // CHECK:   "hls.dataflow.output"(%16) : (tensor<1x32x32x64xi8>) -> ()
    // CHECK: }
    ...
    // CHECK:   %15 = "tosa.avg_pool2d"(%11) {kernel = [32, 32], pad = [0, 0, 0, 0], quantization_info = {input_zp = 0 : i32, output_zp = 0 : i32}, stride = [32, 32]} : (tensor<1x32x32x64xi8>) -> tensor<1x1x1x64xi8>
    ...
    // CHECK: %13 = hls.dataflow.node() -> tensor<1x1x10xi8> {
    ...
    // CHECK:   "hls.dataflow.output"(%17) : (tensor<1x1x10xi8>) -> ()
    // CHECK: }
    ...
    // CHECK:   %15 = "tosa.reshape"(%13) {new_shape = [1, 10]} : (tensor<1x1x10xi8>) -> tensor<1x10xi8>
    // CHECK:   %16 = "tosa.add"(%15, %7) : (tensor<1x10xi8>, tensor<1x10xi8>) -> tensor<1x10xi8>
    // CHECK:   "hls.dataflow.output"(%16) : (tensor<1x10xi8>) -> ()
    // CHECK: }
    ...
    // CHECK: return %14 : tensor<1x10xi8>
